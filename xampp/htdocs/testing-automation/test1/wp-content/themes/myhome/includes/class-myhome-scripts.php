...
//			);
			...
			if ( isset( $_GET['page'] ) && $_GET['page'] == 'myhome_attributes' && is_user_logged_in() ) {
				...
				wp_localize_script(
					'myhome-backend', 'MyHomePanelSettings', array(
		...
		public function load_scripts() {
			...
			if ( ! class_exists( \MyHomeIDXBroker\IDX::class ) ) {
				wp_enqueue_style(
					'myhome-font-awesome', get_template_directory_uri() . '/assets/css/font-awesome.min.css', array(), My_Home_Theme()->version
				);
			}
			...
			if ( is_page_template( 'page_agents.php' ) && class_exists( 'MyHomeCore\Frontend_Panel\User' ) ) {
				...
				$google_api_key   = trim( My_Home_Theme()->settings->get( 'google-api-key' ) );
				$panel_dependency = ! empty( $google_api_key ) ? array(
					'myhome-map',
					'myhome-min'
				) : array( 'myhome-min' );
				...
				wp_localize_script( 'myhome-panel', 'MyHomePanel', array(
					...
					'properties_url'        => admin_url( 'admin-post.php?action=myhome_user_properties' ),
					...
					'force_address_update'  => apply_filters( 'myhome_force_address_update', false ),
			...
			if ( ! empty( $color_primary['color'] ) ) {
				...
				ob_start();
				?>
                :root { --primary: <?php echo esc_html( $color_primary['color'] ); ?>; }
