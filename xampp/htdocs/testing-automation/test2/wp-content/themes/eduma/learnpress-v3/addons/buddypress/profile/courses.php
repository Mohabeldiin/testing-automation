...
$courses = $profile->query_courses( $query, array( 'status' => '','limit' => $limit , 'paged'=>$pages) );
...
if ( $query == 'own' ) {
	if ( ! $courses['total'] ) {
		...
		?>
        <div class="thim-course-grid learn-press-courses profile-courses-list">
			<?php
			global $post;
        ...
        <table class="lp-list-table profile-list-courses profile-list-table">
            ...
            <tr class="list-table-nav">
                ...
                <td colspan="2" class="nav-pages">
					<?php
					 learn_press_buddypress_paging_nav( array('num_pages' => $num_pages ) );
					//$courses->get_nav_numbers( true );
 					?>
                </td>
            </tr>
            </tfoot>
