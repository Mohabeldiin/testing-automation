...
<div id="pmpro_level-<?php echo $pmpro_level->id; ?>">
	<form id="pmpro_form" class="pmpro_form" action="<?php if ( ! empty( $_REQUEST['review'] ) ) {
		...
		<?php if ( $pmpro_review ) { ?>
			<p><?php _e( 'Almost done. Review the membership information and pricing below then <strong>click the "Complete Payment" button</strong> to finish your order.', 'eduma' ); ?></p>
		<?php } ?>
		...
		<table id="pmpro_pricing_fields" class="pmpro_checkout" width="100%" cellpadding="0" cellspacing="0" border="0">
			...
			<tr>
				<th>
					<span class="pmpro_thead-name"><?php _e( 'Membership Level', 'eduma' ); ?></span>
					<?php if ( count( $pmpro_levels ) > 1 ) { ?><span class="pmpro_thead-msg">
						<a href="<?php echo pmpro_url( "levels" ); ?>"><?php _e( 'change', 'eduma' ); ?></a>
						</span><?php } ?>
				</th>
			</tr>
			</thead>
			...
			<tr>
				<td>
					<p>
						<?php printf( __( 'You have selected the <strong>%s</strong> membership level.', 'eduma' ), $pmpro_level->name ); ?>
					</p>
					...
					<?php if ( $pmpro_show_discount_code ) { ?>
						...
						<?php if ( $discount_code && ! $pmpro_review ) { ?>
							<p id="other_discount_code_p" class="pmpro_small">
								<a id="other_discount_code_a" href="#discount_code"><?php _e( 'Click here to change your discount code', 'eduma' ); ?></a>.
							</p>
						<?php } elseif ( ! $pmpro_review ) { ?>
							<p id="other_discount_code_p" class="pmpro_small"><?php _e( 'Do you have a discount code?', 'eduma' ); ?>
								<a id="other_discount_code_a" href="#discount_code"><?php _e( 'Click here to enter your discount code', 'eduma' ); ?></a>.
							</p>
						<?php } elseif ( $pmpro_review && $discount_code ) { ?>
							<p>
								<strong><?php _e( 'Discount Code', 'eduma' ); ?>:</strong> <?php echo $discount_code ?>
							</p>
						<?php } ?>
				...
				<tr id="other_discount_code_tr" style="display: none;">
					<td>
						<div>
							...
							<input type="button" name="other_discount_code_button" id="other_discount_code_button" value="<?php _e( 'Apply', 'eduma' ); ?>" />
						</div>
					</td>
				</tr>
			<?php } ?>
		...
		<?php if ( ! $skip_account_fields && ! $pmpro_review ) { ?>
			<table id="pmpro_user_fields" class="pmpro_checkout" width="100%" cellpadding="0" cellspacing="0" border="0">
				...
				<tr>
					<th>
						<span class="pmpro_thead-name"><?php _e( 'Account Information', 'eduma' ); ?></span>
						<span class="pmpro_thead-msg"><?php _e( 'Already have an account?', 'eduma' ); ?>
							<a href="<?php echo add_query_arg( 'redirect_to', pmpro_url( "checkout", "?level=" . $pmpro_level->id ), thim_get_login_page_url() ); ?>"><?php _e( 'Log in here', 'eduma' ); ?></a></span>
					</th>
				</tr>
				</thead>	
				<tbody>
				...
				<tr>
					<td>
						...
						<span class="gateway_paypal">
							...
							<a href="javascript:void(0);" class="pmpro_radio"><?php _e( 'Check Out with a Credit Card Here', 'eduma' ); ?></a>
						</span>
							...
							<a href="javascript:void(0);" class="pmpro_radio"><?php _e( 'Check Out with PayPal', 'eduma' ); ?></a>
						</span>
						</div>
					</td>
				</tr>
			...
			<table id="pmpro_payment_information_fields" class="pmpro_checkout top1em" width="100%" cellpadding="0" cellspacing="0" border="0" <?php if ( ! $pmpro_requirebilling || apply_filters( "pmpro_hide_payment_information_fields", false ) ) { ?>style="display: none;"<?php } ?>>
				...
				<tr>
					<th>
						...
						<span class="pmpro_thead-msg"><?php printf( __( 'We Accept %s', 'eduma' ), $pmpro_accepted_credit_cards_string ); ?></span>
					</th>
				</tr>
				</thead>
				<tbody>
					...
					<td>
						...
						if ( $pmpro_show_cvv ) { ?>
							<div class="pmpro_payment-cvv">
								<label for="CVV"><?php _e( 'CVV', 'eduma' ); ?></label>
								...
								<small>(<a href="javascript:void(0);" onclick="javascript:window.open('<?php echo pmpro_https_filter( PMPRO_URL ) ?>/pages/popup-cvv.html','cvv','toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=yes, resizable=yes, width=600, height=475');"><?php _e( "what's this?", 'eduma' ); ?></a>)
								</small>
							</div>
						<?php } ?>
						...
						<?php if ( $pmpro_show_discount_code ) { ?>
							<div class="pmpro_payment-discount-code">
								...
								<input type="button" id="discount_code_button" name="discount_code_button" value="<?php _e( 'Apply', 'eduma' ); ?>" />
			...
			<table id="pmpro_tos_fields" class="pmpro_checkout top1em" width="100%" cellpadding="0" cellspacing="0" border="0">
				...
				<tr class="odd">
					<td>
						...
						<label class="pmpro_normal pmpro_clickable" for="tos"><?php printf( __( 'I agree to the %s', 'eduma' ), $tospage->post_title ); ?></label>
					</td>
				</tr>
				</tbody>
			</table>
		...
		<div class="pmpro_submit">
			<?php if ( $pmpro_review ) { ?>
				...
				<input type="submit" class="pmpro_btn pmpro_btn-submit-checkout" value="<?php _e( 'Complete Payment', 'eduma' ); ?> &raquo;" />
			</span>
