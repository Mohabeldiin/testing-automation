...
if ( ! class_exists( 'Thim_Builder_Abstract_Config' ) ) {
	...
	abstract class Thim_Builder_Abstract_Config {
		...
		public function __construct() {
			...
			// set styles inline
			self::$styles_inline = apply_filters( 'thim-builder/' . self::$base . '/styles-inline', $this->get_styles_inline() );
			// set scripts
		 ...
		 * @return  string
		 */
		public function get_styles_inline() {
			return '';
		}
		 ...
		 * @return array
		 */
		public static function _widget_css_inline() {
			$data_css = '';
			if ( self::$styles_inline ) {
				ob_start();
				include_once self::$styles_inline;
				$data_css = ob_get_contents();
				ob_end_clean();
			}
 			return $data_css;
		}
