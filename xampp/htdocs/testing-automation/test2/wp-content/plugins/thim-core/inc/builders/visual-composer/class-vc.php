				...
				foreach ( $_shortcodes as $shortcode ) {
					if ( $group != 'widgets' ) {
						if ( thim_builder_folder_group()) {
							$file = apply_filters( 'thim-builder/vc-shortcode-file', TP_THEME_ELEMENTS_THIM_DIR . "$group/$shortcode/class-vc-$shortcode.php", $shortcode );
						} else {
							$file = apply_filters( 'thim-builder/vc-shortcode-file', TP_THEME_ELEMENTS_THIM_DIR . "$shortcode/class-vc-$shortcode.php", $shortcode );
						}
						if ( file_exists( $file ) ) {
							include_once $file;
						}
